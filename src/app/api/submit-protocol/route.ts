...
export async function POST(req: NextRequest) {
  ...
  if (upErr || !up) {
    return NextResponse.json({ error: upErr?.message ?? "Failed to create protocol" }, { status: 500 });
  }
  ...
  if (catsErr || !catsData) {
    return NextResponse.json({ error: catsErr?.message ?? "Failed to create categories" }, { status: 500 });
  }
  ...
  if (parentErr || !parentData) {
    return NextResponse.json({ error: parentErr?.message ?? "Failed to create items" }, { status: 500 });
  }
